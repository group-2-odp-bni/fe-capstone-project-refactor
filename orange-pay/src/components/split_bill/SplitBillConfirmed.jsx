...
export default function SplitBillConfirmed({
  ...
  useEffect(() => {
      ...
      try {
        ...
        try {
          ...
          if (quotaError.name === "QuotaExceededError") {
            ...
            const toRemove = allEntries.slice(
              0,
              Math.max(1, Math.ceil(allEntries.length * 0.3))
            );
        ...
        try {
          const allSplits = JSON.parse(
            localStorage.getItem("splitBillHistory") || "{}"
          );
          allSplits[splitId] = {
    ...
    if (initialData) {
      const targetUrl = `/app/splitbill/${splitId}?q=${encodeURIComponent(
        initialData?.splitName || "split-bill"
      )}`;
      window.history.replaceState(
        { splitId, data: initialData },
        document.title,
        targetUrl
      );
    }
  ...
  const calculateMemberItemSubtotal = useCallback(
    (memberId) => {
  ...
  const calculateFeeBreakdownPerItem = useCallback(
    (memberId) => {
      ...
      memberItems.forEach((item) => {
        ...
        const itemDiscount =
          Math.abs(displayData.discount || 0) * itemProportionOfTotal;
        const itemService = (displayData.service || 0) * itemProportionOfTotal;
        const itemOther =
          Math.abs(displayData.other || 0) * itemProportionOfTotal;
  ...
  const calculateVerifiedTotal = useCallback(
    (memberId) => {
      ...
      const total =
        ...
        fees.discount +
        fees.service +
        (displayData?.other >= 0 ? fees.other : -fees.other);
      return total;
    },
  ...
  const handleDownloadReceipt = async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(receiptPrintRef.current, {
        ...
        ignoreElements: (el) =>
          el.tagName === "BUTTON" || el.classList.contains("ignore-screenshot"),
      });
    ...
    try {
      ...
      canvas.toBlob(
        async (blob) => {
          ...
          const file = new File([blob], `split-bill-${splitId}.png`, {
            type: "image/png",
          });
          const baseUrl =
            typeof window !== "undefined" ? window.location.origin : "";
          const shareLinkUrl = `${baseUrl}/app/splitbill/${splitId}?q=${encodeURIComponent(
...
Rincian per anggota:
${perMemberVerified
  .map((m, idx) => `${idx + 1}. ${m.name}: ${currency(m.total)}`)
  .join("\n")}
...
Silakan bayar sesuai nominal ya! 🙏
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <p className="text-gray-500">
          Data tidak tersedia. Silakan buat split bill baru.
        </p>
      </div>
    );
  }
  ...
  const popupMemberData = showingDetailFor
    ? perMemberVerified.find((m) => m.id === showingDetailFor)
    : null;
  const popupMemberInfo = showingDetailFor
    ? displayData.members.find((m) => m.id === showingDetailFor)
    : null;
  const popupMemberItems = showingDetailFor
    ? displayData.expandedItems?.filter((item) =>
        item.assignedTo?.includes(showingDetailFor)
      ) || []
    : [];
  ...
  if (showAturPembayaran) {
    const filteredMembers = displayData.members.filter(
      (m) => m.id !== currentUserId && !paymentStatus[m.id]
    );

    const filteredPerMember = perMemberVerified.filter(
      (m) =>
        ...
        >
                                ...
                                <div key={idx}>
                                  <div className="flex items-center justify-between py-3">
                                    <div className="flex items-center gap-3">
                                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#FF9A25] to-[#FF7A25] flex items-center justify-center text-white text-sm font-bold">
                                        {initial}
                                      </div>
                                      <div>
                                        ...
                                        <div className="text-xs text-gray-500">
                                          {phoneDisplay} •{" "}
                                          <span className="text-[#FF8900] font-semibold">
                                            Pembuat
                                          </span>
                                        </div>
                                      </div>
                                    ...
                                    <div className="text-right">
                                      <div className="text-base font-bold text-gray-900">
                                        {memberData
                                          ? currency(memberData.total)
                                          : "-"}
                                      </div>
                                      <div className="text-xs text-[#FF8900] font-semibold">
                                        Lunas
                                      </div>
                                    </div>
                                  </div>
                            ...
                            {(displayData.members || [])
                              ...
                              .map((member, idx) => {
                                ...
                                return (
                                  <div
                                    key={idx}
                                    className="mb-4 pb-4 border-gray-100 last:border-b-0 last:mb-0 last:pb-0"
                                  >
                                    <div className="flex items-center justify-between py-3">
                                      <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#FF9A25] to-[#FF7A25] flex items-center justify-center text-white text-sm font-bold">
                                    ...
                                    <div className="mt-2">
                                      ...
                                      <button
                                        onClick={() => {
                                          ...
                                          alert(
                                            `Link invoice ${member.name} berhasil disalin!`
                                          );
                                        }}
                            ...
                            <div className="border-t-2 border-dashed border-gray-400 pt-4 mt-4 -mx-6 px-6">
                              <button
                                onClick={() => {
                                  ...
                                  setTimeout(
                                    () => setShowAturPembayaran(true),
                                    150
                                  );
                                  setTimeout(() => setClickedButton(null), 300);
                                }}
                                className={`w-full py-4 md:py-4 rounded-3xl font-bold text-sm text-white transition-all duration-200 flex items-center justify-center gap-2.5 active:scale-95 ${
                                  clickedButton === "aturPembayaran"
          ...
          >
            ...
            >
              ...
              {/* TOTAL */}
              <div
                style={{ padding: "16px 18px 10px 18px", textAlign: "center" }}
              >
                <div
                  style={{
                    fontSize: 12,
          ...
          <div className="px-4 pb-6 space-y-3 max-w-md mx-auto w-full mt-5">
            <button
              ...
              className={`w-full py-3.5 rounded-xl font-semibold text-sm bg-white border-2 border-gray-200 text-gray-700 active:scale-[0.98] transition-all ${
            ...
            <div className="p-5">
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="font-semibold text-gray-900">
                        {currency(roundIDR(totalForThisPerson))}
                      </span>
                    </div>
                  );
                })}
              ...
              <div className="border-t-2 border-gray-200 pt-3 space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-gray-900">
                    {currency(popupCalculatedSubtotal)}
                  </span>
                </div>
                {(() => {
                  ...
                  return (
                    <>
                      ...
                      <div className="flex justify-between text-sm">
                        ...
                        >
                          {(fees.discount || 0) > 0
                            ? `-${currency(roundIDR(fees.discount))}`
                            : currency(0)}
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-semibold text-gray-900">
                          {currency(roundIDR(fees.service || 0))}
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
                        ...
                        <span
                          className={`font-semibold ${
                            (displayData.other || 0) < 0
                              ? "text-red-600"
                              : "text-gray-900"
                          }`}
                        >
                          {(displayData.other || 0) < 0
                            ? `-${currency(roundIDR(fees.other))}`
                            : currency(roundIDR(fees.other || 0))}
                        </span>
                      </div>
                    </>
                  );
                ...
                <div className="flex justify-between text-base pt-2 border-t border-gray-200">
                  ...
                  <span className="font-bold text-[#FF9A25]">
                    {currency(popupCalculatedTotal)}
                  </span>
                </div>
              </div>
            </div>
