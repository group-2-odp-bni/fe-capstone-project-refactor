...
export default function SplitBillConfirmation({
    ...
    Object.keys(grouped).forEach((key) => {
      ...
      if (group.length === 1 || allUnassigned) {
        ...
        if (unassigned.length > 0) {
          const totalUnassignedQty = unassigned.reduce(
  ...
  const assignItemWithQuantity = (expandedIdx, memberId, qty) => {
    setExpandedItems((prev) => {
      ...
      if (!someonePickedMax && availableQty > 0) {
        ...
        const newList = splitItemIfNeeded(
          prev,
          expandedIdx,
          finalQty,
          memberId
  ...
  const getMemberAssignmentCount = (memberId) => {
    return expandedItems.filter((item) => item.assignedTo.includes(memberId))
      .length;
  };
  ...
  const countedSubtotal = useMemo(() => {
    ...
    expandedItems.forEach((item) => {
      ...
      const itemTotal = Object.values(item.assignedQuantities || {}).reduce(
        (s, q) => s + q * item.pricePerUnit,
        0
      );
  ...
  const perMember = useMemo(() => {
    ...
    const originalItemTotals = items.map((it) =>
      Number(it.line_subtotal_rp || 0)
    );
    ...
    return result;
  }, [memberIds, rawAdjustSum, expandedItems, subtotal, items]);
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 overflow-auto px-4 py-4">
        <div
          className={`max-w-3xl mx-auto space-y-4 transition-all duration-500 ${
        ...
        >
          <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
                ...
                {members.map((m, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className={`w-16 h-16 sm:w-[68px] sm:h-[68px] rounded-full flex items-center justify-center shadow-md text-white text-xl sm:text-2xl font-bold transition-all relative z-10 ${
                          isSelected
                      ...
                      <div
                        className={`mt-2 w-20 sm:w-24 text-[10px] sm:text-[11px] truncate text-center transition-colors leading-tight ${
                          isSelected
                            ? "text-[#FF9A25] font-bold"
                            : "text-gray-700 group-hover:text-[#FF9A25] font-medium"
                        }`}
                      >
                        {m.name || "—"}
                      </div>
              ...
              <div className="mt-5 flex justify-center">
                {selectedMemberId ? (
                  <div className="inline-flex items-center gap-2 px-4 h-10 rounded-full bg-gradient-to-r from-[#FFF3E6] to-[#FFEDD5] border-2 border-[#FF9A25] text-xs font-semibold text-[#B45309] shadow-md">
                    ...
                    {
                      members.find(
                        (m) => (m.id ?? m.name ?? m.phone) === selectedMemberId
                      )?.name
                    }
                  </div>
                ) : (
                ...
                >
                  <div className="bg-white px-6 py-4 rounded-2xl shadow-xl border-2 border-[#FF9A25] text-center max-w-xs mx-4 animate-in zoom-in-95 duration-200">
                    <svg
                      ...
                      height="48"
                      viewBox="0 0 24 24"
                      className="mx-auto mb-3 text-[#FF9A25]"
                    >
                      <path
                        d="M12 15v2m0 0v2m0-2h2m-2 0h-2"
                        stroke="currentColor"
                        strokeWidth="2"
                        strokeLinecap="round"
                      />
              ...
              <div className="divide-y-2 divide-dashed divide-gray-300">
                {expandedItems.map((item, expandedIdx) => {
                  const isAssignedToSelected =
                    item.assignedTo.includes(selectedMemberId);
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                              ...
                              <div className="flex flex-wrap gap-1.5">
                                {item.assignedTo.map((memberId) => {
                                  const assignedMember = members.find(
                                    (m) =>
                                      (m.id ?? m.name ?? m.phone) === memberId
                                  );
                                  const assignedQty =
                                    item.assignedQuantities[memberId] || 0;
                                  if (!assignedMember || assignedQty === 0)
                                    return null;
                                  return (
                                    <div
                                      key={memberId}
                        ...
                        <div
                          className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all flex-shrink-0 shadow-sm ${
            ...
            <div
              className="h-0 border-t-2 border-dashed border-gray-500 mb-3"
              aria-hidden
            />
            ...
            <div className="pb-6">
              <div
                className="h-0 border-t-2 border-dashed border-gray-500 my-2"
                aria-hidden
              />
            </div>
              ...
              <div className="mx-auto max-w-3xl px-4">
                <div className="w-full rounded-2xl bg-gradient-to-r from-[#A76C29] to-[#8B5A1F] text-white text-xs font-semibold px-4 py-3 text-center border-2 border-[#FF8900] shadow-lg">
                  {expandedItems.length ? (
                    <>
                      {fullyAssignedCount} dari {expandedItems.length} pesanan
                      dihitung
                      {notCounted > 0 && (
                    ...
                    <span className="text-[#FFD900]">
                      Belum ada pesanan yang terdeteksi
                    </span>
                  )}
                </div>
              </div>
...
if (
  ...
  !document.getElementById("sb-style-hide")
) {
