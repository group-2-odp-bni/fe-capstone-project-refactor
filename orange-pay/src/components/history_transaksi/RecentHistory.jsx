...
import { useWalletHistoryByMonth } from "../../hooks/api/useHistory"; // named import
...
export default function RecentHistory({ walletId, onExpandChange, dynamicTop }) {
  const navigate = useNavigate();

  // define before using in hook
  const [activeMonth, setActiveMonth] = useState(getMonthShort(new Date()));
  const year = useMemo(() => new Date().getFullYear(), []);

  // fetch month-scoped history for this wallet
  const { users = [], loading } = useWalletHistoryByMonth({
    walletId,
  ...
  // ✅ do NOT filter by walletId again (items don't carry walletId)
  ...
  const handleTouchMove = (e) => {
    ...
    const newTop = Math.min(Math.max(startTop.current + delta, 80), BOTTOM_LIMIT_PX);
    setSheetTop(newTop);
  };
  ...
  // open the receipt page for this transaction
  const handleTransactionClick = (item) => {
    const trxId = item?.trxId || item?.id; // support both shapes
    if (!trxId) return;
  
    navigate(`/app/receipt/${encodeURIComponent(trxId)}`, {
      state: {
        walletId,         // handy on the receipt page
        transfer: item,   // pass the item if you want instant UI before refetch
      },
      replace: false,
    });
  };
    ...
    <section className="relative bg-transparent">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-6 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
          ...
          <h2 className="text-sm font-medium text-gray-600 mt-8 mb-2">Riwayat Transaksi</h2>
          ...
          {loading ? (
            ...
            <div className="py-6 text-center text-sm text-gray-500">Belum ada transaksi.</div>
          ) : (
