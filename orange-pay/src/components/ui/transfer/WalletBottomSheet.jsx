// src/components/ui/transfer/WalletBottomSheet.jsx
...
export default function WalletBottomSheet({
  open = true,
  onClose,
  onSelect,           // expects onSelect(wallet)
  wallets = [],       // passed from parent (filtered, no "add-card")
  selectedId,        // currently selected wallet id
  dynamicTop,
}) {
  // Top positioning
  ...
  // Normalize items from parent
  const items = useMemo(
    () =>
      (Array.isArray(wallets) ? wallets : []).map((w) => ({
        id: w.id,
        name: w.walletName || w.title || "Wallet",
        balance: Number(w.balance ?? 0),
        type: w.type || (w.defaultForUser ? "Utama" : "Personal"),
        defaultForUser: Boolean(w.defaultForUser),
        _raw: w,
      })),
    [wallets]
  );
    ...
    <section className="relative">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-6 divide-y divide-gray-100">
          {items.length === 0 ? (
            <div className="py-6 text-center text-sm text-gray-400">No wallets</div>
          ) : (
            items.map((w) => {
              const selected = selectedId && w.id === selectedId;
              return (
                <button
                  key={w.id}
                  type="button"
                  ...
                  onClick={() => onSelect?.(w._raw)}   // ← pass the wallet object
                >
                  <div className="min-w-0 flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <div className="text-sm font-medium truncate">
                        {w.name}
                        {w.defaultForUser && (
                          <span className="ml-2 text-[10px] px-1.5 py-0.5 rounded bg-orange-100 text-orange-700 align-middle">
                            Utama
                          </span>
                        )}
                      </div>
